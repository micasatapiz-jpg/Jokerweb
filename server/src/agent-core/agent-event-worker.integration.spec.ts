...
describe(
  'AgentEventWorkerService integration',
  () => {
    let db:
      PrismaService
    ...
    it(
      'reanuda un workflow pendiente desde un evento persistido',
      async () => {
        if (
          !db
        ) {
        ...
        /*
         * processPendingEvents solamente
         * consume el evento y reanuda.
         *
         * No ejecuta la fase posterior
         * de workflows RUNNING.
         */
        const result =
    ...
    it(
      'un segundo ciclo no repite el mismo evento consumido',
      async () => {
        if (
          !db
        ) {
    ...
    it(
      'crea TIME_REACHED para workflow vencido y lo reanuda',
      async () => {
        if (
          !db
        ) {
         ...
         *
         * 1. crea TIME_REACHED;
         * 2. reanuda el workflow;
         * 3. continúa los steps RUNNING;
         * 4. completa el workflow si
         *    ya no quedan steps.
         */
        const result =
    ...
    it(
      'no crea TIME_REACHED para un timer futuro',
      async () => {
        if (
          !db
        ) {
    ...
    it(
      'reanuda automáticamente un evento pendiente al iniciar el worker',
      async () => {
        if (
          !db
        ) {
        ...
        try {
          ...
          /*
           * El workflow puede pasar por RUNNING
           * demasiado rápido para que el test
           * alcance a observar ese estado.
           *
           * El resultado durable esperado,
           * al no quedar pasos pendientes,
           * es COMPLETED.
           */
          while (
            ![
          ...
          /*
           * Si alcanzamos RUNNING justo en
           * medio del ciclo, damos un pequeño
           * margen para que el worker termine
    ...
    it(
      'dos ciclos automáticos no duplican el mismo resume',
      async () => {
        if (
          !db
        ) {
    ...
    it(
      'dos réplicas concurrentes consumen el mismo evento una sola vez',
      async () => {
        if (
          !db
        ) {
         ...
         *
         * Esto representa dos instancias
         * reales intentando consumir
         * el mismo evento.
         */
        const replicaConfig =
          new ConfigService({
        ...
        try {
           ...
           *
           * FOR UPDATE sobre AgentEvent
           * debe serializar la carrera.
           */
          const [
            first,
            second,
           ...
           * resumeFromEvent().
           *
           * No se ejecutó processOnce(),
           * por lo tanto el workflow debe
           * seguir RUNNING.
           */
          const current =
            await orchestrator.getWorkflow(
