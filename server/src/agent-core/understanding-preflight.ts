...
export async function understandingPreflight(tx:Prisma.TransactionClient,tenantId:string,chat:Conversation,turn:AgentTurn,sourceIds:string[],recordOnly=false) {
  ...
  if(['COMPOSITE','SPECIAL','POSSIBLE_OUTSOURCING'].includes(understanding.productResolution)||policy.decision==='NEEDS_OWNER_KNOWLEDGE'&&understanding.requestComponents.some(c=>!c.matchedProductId)) {
    ...
    await tx.ownerReview.upsert({where:{tenantId_dedupeKey:{tenantId,dedupeKey}},create:{tenantId,conversationId:chat.id,sourceMessageId:first.id,reason:'COMMERCIAL_KNOWLEDGE_REQUIRED',risk:30,recommendedAction:'KEEP_ACTIVE',dedupeKey,details},update:{}})
    await tx.task.upsert({where:{tenantId_dedupeKey:{tenantId,dedupeKey}},create:{tenantId,jobId,conversationId:chat.id,type:'CHECK_PRODUCT_RULE',title:'OWNER: completar conocimiento comercial',dedupeKey,details},update:{}})
  ...
  if(understanding.primaryGoal==='CANCELLATION') return {status:'CANCELLATION_REVIEW',reply:'¿Qué trabajo deseas cancelar? No he cancelado ningún pedido todavía.',tools:[]}
